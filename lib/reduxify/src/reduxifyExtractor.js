...
reduxify.reducersExtractor = (reducers) => {
  ...
  if (strReducers.includes('switch')) {
    ...
    for (let i = 0; i < reducerCases.length; i++) {
      if (reducerCases[i]) {
        for (let j = 0; j < reducerCases[i].length; j++) {
          reducerCases[i][j] = reducerCases[i][j]
          .replace(/case|break|return/g, '')
          .replace(/['":;,\s]/g, '');
        }
      }
    }
    ...
    let reducerCases = strReducers.split('action.type').slice(1).map(subStr => subStr.replace(/[^\w\\)]/g, '')).map(subStr => subStr = subStr.slice(0, subStr.indexOf(')')));  
    return assembleVerifiedReducerNodes(headNode, reducers, reducerNames, reducerCases);
  }
...
function assembleReducerNodes(headNode, reducers, reducerNames, reducerCases) {
  reducerNames.forEach((reducer, i) => {
    ...
    if (reducerCases[i]) reducerCases[i].forEach(c => reducerNode.children.push(new Node(c)));
...
function assembleVerifiedReducerNodes(headNode, reducers, reducerNames, reducerCases) {
  reducerNames.forEach((reducer, i) => {
    const reducerNode = new Node(reducer);
    reducerNode.children = [];
      if (reducerCases[i]) { 
