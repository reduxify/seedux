...
/**
* Calls the next dispatch method in the middleware chain, attaches undo and redo event listeners to the application, and creates a custom event object containing aggregate information regarding the application's old and new state.
*
* @param {Object} dispatch is a function that dispatches  actions in order to change the state of the application.
*
* @param {Object} getState is a function that returns the current state of the Redux store.
*
* @returns {Object} modifiedAction is an object containing the original action, the modified action (if any), the new state, and any differences between the old state and new state.
*/
