  ...
  if (uiPropNames) {
    if (!mapStateToPropsDef && !mapDispatchToPropsDef) {
      ...
      if (mapStateToPropsDef.includes('state.')) {
        ...
        uiActionCreators = uiActionCreators.split('dispatch(').slice(1).map(subStr => subStr.slice(subStr.indexOf('.') + 1, subStr.indexOf(')')));
    ...
    uiPropNames = uiPropNames.split(':').map(subStr => subStr.slice(-1 * ((subStr.length - 1) - subStr.lastIndexOf(' ')))).filter(subStr => /[\w*]/gi.test(subStr));
  }
...
function reducersExtractor(reducers) {
  ...
  let reducerCases;
  console.log('found reducers: ', reducers, reducerNames);
...
function actionCreatorsExtractor(actionCreators) {
  console.log('Got actionCreators: ', actionCreators);
